...
import type { BrowserCookieQuery } from "./browser-cookie-types.js";
...
export interface FirefoxReadOptions extends OpenBrowserStoreOptions {
  /** Host selection; defaults to the QwenCloud console domains. */
...
async function queryFirefoxCookies(
  ...
  try {
    ...
    const hostFilter = options?.query?.hostSql ?? firefoxCookieHostSql();
    const acceptHost = options?.query?.acceptHost ?? isQwenCloudCookieDomain;
    ...
    return rows.flatMap((row) => {
      ...
      if (typeof row.host !== "string" || !acceptHost(row.host)) return [];
